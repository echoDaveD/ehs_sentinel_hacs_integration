...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    for key, value in coordinator.data.get(PLATFORM_SELECT, {}).items():
        ...
        coordinator.data[PLATFORM_SELECT][key]['_entity'] = entity  # speichere die entity im coordinator.data
...
class EHSSentinelSelect(CoordinatorEntity, SelectEntity, RestoreEntity):
    def update_value(self, valuedict):
        ...
        old_value = self.coordinator.data.get(PLATFORM_SELECT, {}).get(self._key, {}).get('value')
        old_nasa_name = self.coordinator.data.get(PLATFORM_SELECT, {}).get(self._key, {}).get('nasa_name', None)
        old_nasa_seen = self.coordinator.data.get(PLATFORM_SELECT, {}).get(self._key, {}).get('nasa_last_seen', None)
        old_seen_once = self.coordinator.data.get(PLATFORM_SELECT, {}).get(self._key, {}).get('seen_once', False)
        if old_value != value or old_nasa_name != nasa_name or old_nasa_seen != nasa_last_seen or old_seen_once != seen_once:
            self.coordinator.data[PLATFORM_SELECT][self._key]['value'] = value
            self.coordinator.data[PLATFORM_SELECT][self._key]['nasa_name'] = nasa_name
            self.coordinator.data[PLATFORM_SELECT][self._key]['nasa_last_seen'] = nasa_last_seen
            self.coordinator.data[PLATFORM_SELECT][self._key]['seen_once'] = seen_once
            if self.hass:
    ...
    def __init__(self, coordinator, key, nasa_name=None):
        ...
        self._options = hass_opts.get('platform', {}).get("options", [])
