...
from .const import DOMAIN, DEVICE_ID, PLATFORM_BINARY_SENSOR

async def async_setup_entry(hass, entry, async_add_entities):
    coordinator = hass.data[DOMAIN][entry.entry_id]
    coordinator.register_entity_adder(PLATFORM_BINARY_SENSOR, async_add_entities)
    await coordinator.async_config_entry_first_refresh()
    entities = []
    for key, value in coordinator.data.get(PLATFORM_BINARY_SENSOR, {}).items():
        entity = EHSSentinelBinarySensor(coordinator, key, nasa_name=value.get("nasa_name"))
        entities.append(entity)
        coordinator.data[PLATFORM_BINARY_SENSOR][key]['_entity'] = entity  # speichere die entity im coordinator.data
    async_add_entities(entities)

class EHSSentinelBinarySensor(CoordinatorEntity, BinarySensorEntity, RestoreEntity):
    def update_value(self, valuedict):
        ...
        old_value = self.coordinator.data.get(PLATFORM_BINARY_SENSOR, {}).get(self._key, {}).get('value')
        old_nasa_name = self.coordinator.data.get(PLATFORM_BINARY_SENSOR, {}).get(self._key, {}).get('nasa_name', None)
        old_nasa_seen = self.coordinator.data.get(PLATFORM_BINARY_SENSOR, {}).get(self._key, {}).get('nasa_last_seen', None)
        old_seen_once = self.coordinator.data.get(PLATFORM_BINARY_SENSOR, {}).get(self._key, {}).get('seen_once', False)
        if old_value != value or old_nasa_name != nasa_name or old_nasa_seen != nasa_last_seen or old_seen_once != seen_once:
            self.coordinator.data[PLATFORM_BINARY_SENSOR][self._key]['value'] = value
            self.coordinator.data[PLATFORM_BINARY_SENSOR][self._key]['nasa_name'] = nasa_name
            self.coordinator.data[PLATFORM_BINARY_SENSOR][self._key]['nasa_last_seen'] = nasa_last_seen
            self.coordinator.data[PLATFORM_BINARY_SENSOR][self._key]['seen_once'] = seen_once
            if self.hass:
    ...
    async def async_added_to_hass(self):
        ...
        if last_state:
            ...
            platform_data = self.coordinator.data.setdefault(PLATFORM_BINARY_SENSOR, {})
            platform_data.setdefault(self._key, {})

            # Konvertiere zu float/int und prüfe min/max
            state_val = last_state.state
            if isinstance(state_val, str):
                state_val = state_val.lower() in ("on", "true", "1", "yes")
    ...
    @property
    def is_on(self):
        return self.coordinator.data.get(PLATFORM_BINARY_SENSOR, {}).get(self._key).get("value") in (True, "on", "ON", 1)

    @property
    def available(self):
        data = self.coordinator.data.get(PLATFORM_BINARY_SENSOR, {}).get(self._key, {})
